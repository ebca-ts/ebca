...
import type {
  ...
  EbcaGqlRequestComponentsPayload,
...
import { serializeEbcaGqlJsonObject } from '../utils/ebca-gql-json';
import { EbcaGqlProjectionService } from './ebca-gql-projection.service';

@Injectable()
export class EbcaGqlComponentRequestService {
  private readonly logger = new Logger(EbcaGqlComponentRequestService.name);
  ...
  async resolveRequestedComponents(
    ...
    payload: EbcaGqlRequestComponentsPayload,
  ): Promise<EbcaGqlEbcaComponentPayload[]> {
    const targets = payload.targets.slice(
      0,
      this.options.limits.maxComponentRequestTargets,
    );
    const result: EbcaGqlEbcaComponentPayload[] = [];
  ...
  private async resolveEntityTarget(
    identity: EbcaGqlAuthenticatedIdentity,
    target: EbcaGqlRequestComponentsPayload['targets'][number],
  ): Promise<EbcaGqlEbcaComponentPayload[]> {
    if (!target.entityId) {
      return [];
    }
  ...
  private async resolveCollectionTarget(
    ...
    if (!entityIds) {
      ...
      if (target.ownedOnly) {
        ...
        if (!filtered) {
          this.logger.warn(
            `Owned EBCA GraphQL collection query for ${entityClass.name} has no identity projection column.`,
          );
        }
      }
  ...
  private resolveEntityClass(
    ...
    try {
      ...
      this.logger.warn(
        `Rejected EBCA GraphQL component request for unregistered entity ${entityName}.`,
      );
      return null;
    }
  }
  ...
  private resolveComponentClass(
    ...
    try {
      ...
      this.logger.warn(
        `Rejected EBCA GraphQL component request for unregistered component ${componentName}.`,
      );
      return null;
    }
  }
  ...
  private resolveRepository(
    ...
    try {
      ...
      this.logger.warn(
        `Skipped EBCA GraphQL collection query for ${entityClass.name}: repository is not available.`,
      );
      return null;
    }
  }
