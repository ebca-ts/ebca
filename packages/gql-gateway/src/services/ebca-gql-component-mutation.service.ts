...
import { EBCA_GQL_INBOUND_NORMALIZERS } from '../tokens';
import type {
  EbcaGqlComponentMutationPayload,
...
import type {
  ...
  EbcaGqlComponentFieldValue,
  EbcaGqlInboundNormalizer,
  EbcaGqlInboundNormalizerContext,
} from '../types/ebca-gql-gateway.options';
import {
  cloneMutableEbcaGqlJsonObject,
  resolveEbcaGqlStringList,
  serializeEbcaGqlJsonObject,
} from '../utils/ebca-gql-json';

type HydratedComponentRecord = BaseComponent &
  Record<string, EbcaGqlComponentFieldValue>;

@Injectable()
export class EbcaGqlComponentMutationService {
  private readonly logger = new Logger(EbcaGqlComponentMutationService.name);

  constructor(
    private readonly componentManager: ComponentManager,
    @Inject(EBCA_GQL_INBOUND_NORMALIZERS)
    private readonly normalizers: readonly EbcaGqlInboundNormalizer[],
  ) {}

  async applyMutation(
    identity: EbcaGqlAuthenticatedIdentity,
    payload: EbcaGqlComponentMutationPayload,
  ): Promise<EbcaGqlComponentMutationResultPayload> {
    ...
    if (!inbound?.expose) {
      throw new ForbiddenException(
        `Component ${payload.componentName} is not open for GraphQL inbound.`,
      );
    }
    ...
    if (component instanceof BaseCommandComponent) {
      ...
      component.commandSource = CommandComponentSource.GRAPHQL;
      ...
      this.logger.debug(
        `Applied GraphQL ${payload.operation} command ${payload.componentName} on ${payload.entityName}:${payload.entityId} from identity ${identity.identityId}.`,
      );
      return this.accepted(payload);
    }
    ...
    this.logger.debug(
      `Applied GraphQL ${payload.operation} for ${payload.componentName} on ${payload.entityName}:${payload.entityId} from identity ${identity.identityId}.`,
    );
    return this.accepted(payload);
  }
  ...
  private assertOperationAllowed(
    operation: EbcaGqlComponentMutationPayload['operation'],
  ...
  private assertFieldsDeclared(
    ...
    throw new BadRequestException(
      `GraphQL inbound component ${payload.componentName} must declare inbound.fields.`,
    );
  }
  ...
  private async assertEntityScope(
    ...
    if (!inbound.ownerComponent) {
      throw new BadRequestException(
        `GraphQL inbound ownerField for ${payload.componentName} requires ownerComponent.`,
      );
    }
    const entity = new entityClass();
    entity.id = payload.entityId;
  ...
  private async resolveOwnerComponent(
    ...
    return ownerComponent ? serializeEbcaGqlJsonObject(ownerComponent) : {};
  }
  ...
  private hydrateComponent(
    ...
    const mutableComponent = cloneMutableEbcaGqlJsonObject(rawComponent);
  ...
  private normalizeInboundDate(field: string, value: EbcaGqlJsonValue): Date {
